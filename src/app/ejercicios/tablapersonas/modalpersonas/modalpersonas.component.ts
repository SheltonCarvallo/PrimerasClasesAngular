...
export class ModalpersonasComponent {
  ...
  @Output()
  eventoGuardar = new EventEmitter<Persona>();

  @ViewChild(Modal, {static : true}) divModal!: Modal; //I don't have fucking idea of this
  ...
  validarGuardar(): boolean {
    ...
    if (this.validarEstaVacio(this.dataPersona.apellidos)) {
      this.mostrarMensajeError("El valor de apellido no se ha agregado!!");
      return true;
    }
  ...
  mostrarMensajeExito(message: string) {
    this.mostrarMensajeBase(message, 'sucess');
  }
